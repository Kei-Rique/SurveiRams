...
const actions = {
  ...
  fbReadData({ commit }) {
    let userAssignments = projectDb.ref(
      "assignments/O6IWng6uOiWp83OkYVlvxZ4Dcft2"
    );

    //child added
    userAssignments.on("child_added", (snapshot) => {
      let assignment = snapshot.val();
      let payload = { [snapshot.key]: assignment };
  ...
  fbAddAssignment({}, payload) {
    let assignmentRef = projectDb.ref(
      "assignments/O6IWng6uOiWp83OkYVlvxZ4Dcft2/" + payload.id
    );
    assignmentRef.set(payload.assignment);
  },
  fbUpdateAssignment({}, payload) {
    let assignmentRef = projectDb.ref(
      "assignments/O6IWng6uOiWp83OkYVlvxZ4Dcft2/" + payload.id
    );
    assignmentRef.update(payload.updates);
  },
  fbDeleteAssignment({}, assignmentId) {
    let assignmentRef = projectDb.ref(
      "assignments/O6IWng6uOiWp83OkYVlvxZ4Dcft2/" + assignmentId
    );
...
const getters = {
  ...
  assignmentsCompleted: (state, getters) => {
    ...
    Object.keys(assignmentsFiltered).forEach(function (key) {
      let assignment = assignmentsFiltered[key];
      if (assignment.completed && !assignment.feedback) {
        assignments[key] = assignment;
      }
    });
    return assignments;
  },
  assignmentsClosed: (state, getters) => {
    let assignmentsFiltered = getters.assignmentsFiltered;
    let assignments = {};
    Object.keys(assignmentsFiltered).forEach(function (key) {
      let assignment = assignmentsFiltered[key];
